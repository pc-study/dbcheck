...
-- | USAGE    :                                                                         |
-- | sqlplus / as sysdba @dbcheck12c.sql                                                |
-- |                                                                                    |
...
COLUMN CDB NEW_VALUE _CDB NOPRINT
COLUMN PDBS NEW_VALUE _PDBS NOPRINT
SELECT DECODE((SELECT b.parallel FROM v$instance b), 'YES', (d.NAME || '_' ||  (SELECT b.INSTANCE_NUMBER FROM v$instance b)), 'NO', d.NAME)   dbname1,  
       ...
       decode(d.cdb,'YES',d.cdb || ', PDBS: ' ||  (SELECT listagg(pdb_name,', ') within group(order by pdb_id) pdbs FROM dba_pdbs where pdb_name <> 'PDB$SEED'),d.cdb) pdbs
  FROM  v$database d;
...
SELECT 'Status: ' || a.VALUE || ', Used_Size: ' ||
       (SELECT round(SUM(a.space * (SELECT value
          ...
          FROM cdb_recyclebin a) recyclebin
  FROM v$parameter a
...
SELECT trim(count(*)) TBS_CNT FROM cdb_tablespaces;
...
SELECT trim(count(*)) USER_CNT from cdb_users where con_id not in (1,2) and default_tablespace not in ('SYSTEM','SYSAUX') and account_status = 'OPEN';
...
SELECT 'DB Files Count: ' || ((SELECT count(*) FROM cdb_data_files) + (SELECT count(*) FROM cdb_temp_files)) || ', DB Files Limit: ' || value DBFILES FROM v$parameter WHERE name = 'db_files';
...
<tr><td style="color:White; background:#0066cc;" width="200"><b>CDB?</b></td><td>&_pdbs</td></tr> - 
...
SELECT (SELECT round(SUM(bytes)/1024/1024/1024,2) FROM cdb_data_files) ts_datafile_physical_size_G,
       (SELECT round(SUM(bytes)/1024/1024/1024,2) FROM cdb_temp_files) ts_tempfile_physical_size_G,
       (SELECT round(SUM(bytes)/1024/1024/1024,2) FROM cdb_segments) ts_datafile_used_size_G
  FROM dual;
...
SELECT v.rtime,
       ...
       CASE
         WHEN v.pct_used >= 90 THEN
          '<div align="right"><font color="red"><b>' || to_char(v.pct_used,
                                                                '990.99') || '</b></font></div>'
         ELSE
          '<div align="right"><font color="green"><b>' || to_char(v.pct_used,
                                                                  '990.99') || '</b></font></div>'
       END pct_used
  ...
  FROM (SELECT t.owner,
         ...
         WHERE t.owner = s.owner
           ...
           AND s.tablespace_name NOT IN ('SYSTEM',
                                         'SYSAUX')
           AND s.owner IN (SELECT username FROM cdb_users WHERE oracle_maintained = 'N' AND con_id NOT IN (1,2))
         GROUP BY t.owner,
                  t.table_name,
                  t.partitioned,
                  t.table_name
...
WITH this_part AS (
  ...
  JOIN (
    ...
    FROM cdb_tab_partitions b
    JOIN cdb_users du ON b.table_owner = du.username
...
all_part AS (
  ...
  FROM CONTAINERS(sys.tabpart$) tp
  INNER JOIN cdb_objects o ON tp.obj# = o.object_id
  INNER JOIN cdb_users u ON o.owner = u.username
  INNER JOIN this_part t2 ON t2.partition_name = o.subobject_name 
                           AND t2.table_owner = u.username 
                           AND o.object_name = t2.table_name
  WHERE u.username NOT IN ('SYS', 'SYSTEM', 'DBSNMP', 'MGMT_VIEW', 'SYSMAN', 'SH', 'MDSYS', 'AUDSYS')
  AND tp.con_id NOT IN (1,2)
)
SELECT t2.username,
       t2.tablename,
       t2.partname,
...
SELECT u.username as username,
       ...
       TO_NUMBER(SUBSTR(RAWTOHEX(CAST(tp.bhiboundval AS RAW(8))), 15, 2),
                 'XX') - 1 as ss
  from cdb_objects o, CONTAINERS(sys.tabcompart$) tp, cdb_users u, tab$ t, this_part2 t2
 where tp.obj# = o.object_id
   ...
   AND u.username NOT IN ('SYS',
                      ...
                      'MDSYS',
                      'AUDSYS')
   and t2.partition_name = o.subobject_name 
   and t2.table_owner = u.username 
   AND tp.con_id NOT IN (1,2)) ttt
...
SELECT owner,
 ...
 WHERE tablespace_name = 'SYSTEM'
   AND owner IN (SELECT username FROM cdb_users WHERE oracle_maintained = 'N' AND con_id NOT IN (1,2));
...
SELECT u.username,
  ...
  FROM cdb_users u
  LEFT JOIN cdb_objects o ON o.owner = u.username AND o.con_id = u.con_id
  LEFT JOIN cdb_tables t ON t.owner = o.owner AND t.table_name = o.object_name AND o.object_type = 'TABLE' AND t.con_id = o.con_id
 WHERE u.account_status = 'OPEN'
   AND u.con_id NOT IN (1,2)
   AND u.oracle_maintained = 'N'
 GROUP BY u.username
 ORDER BY u.username;

host echo start collect......Stale Statistics Check...

prompt <font size="+1" face="Consolas" color="#336699"><b>● Tables with Stale Statistics</b></font>

CLEAR COLUMNS COMPUTES
SET DEFINE OFF
SET MARKUP html TABLE 'id="stalestats" border="1" width="90%" align="center" summary="Script output" '

COLUMN owner           HEADING 'Owner'
COLUMN table_name      HEADING 'Table Name'
COLUMN num_rows        HEADING 'Num Rows'
COLUMN last_analyzed   HEADING 'Last Analyzed'
COLUMN stale_stats     HEADING 'Stale?'

SELECT s.owner,
       s.table_name,
       s.num_rows,
       TO_CHAR(s.last_analyzed, 'YYYY-MM-DD HH24:MI:SS') last_analyzed,
       '<font color="red"><b>' || s.stale_stats || '</b></font>' stale_stats
...
SELECT *
  FROM (SELECT n.owner,
          ...
          FROM cdb_scheduler_job_log         n,
               cdb_scheduler_job_run_details jrd
         WHERE n.log_id = jrd.log_id(+)
           AND n.con_id not in (1,2)
...
SELECT w.window_name,
       ...
       CASE
         ...
         ELSE
          '<div align="left"><font color="green"><b>' || w.enabled || '</b></font></div>'
       END enabled
  FROM cdb_autotask_window_clients c,
       cdb_scheduler_windows       w
 WHERE c.window_name = w.window_name
   AND c.con_id not in (1,2)
...
COLUMN  cluster_name        HEADING  'Cluster Name'
COLUMN  status              HEADING  'Status'
